    ...
    for gitCommit in compare.commits:
        ...
        if featureId is not None:
            if featureId in featureIds:
                ...
                set_pivotal_project_delivered(pivotalKey, pivotalProjects, featureId)
...
def set_pivotal_project_delivered(token, pivotalProjects, storyId):
    for pivotalProject in pivotalProjects:
    	current_state = {'current_state': 'delivered'}
        response = requests.put("https://www.pivotaltracker.com/services/v5/projects/{0}/stories/{1}".format(pivotalProject.id, storyId), headers={'X-TrackerToken': token}, params=current_state)
